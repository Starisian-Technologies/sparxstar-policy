...
**9.1 User-Prompted Outputs.** For AI-Assisted Outputs generated in response to specific User prompts or User-submitted inputs: (a) the User retains all rights they hold in the original prompt, input content, and any creative contributions they made to the output; (b) to the extent the AI-Assisted Output constitutes a copyrightable work under applicable law, the allocation of rights between the User and the Platform is governed by the applicable service terms for the specific AI Feature used; (c) where no specific service terms address the issue, the Platform and User share rights in the output proportionate to their respective creative contributions, to the maximum extent permitted by applicable law.

**9.2 Platform-Generated Outputs.** AI-Assisted Outputs generated by Platform systems autonomously — without a specific User prompt and not derived from a specific User's Content — are Platform Content owned by the Company. This includes: (a) Platform-generated Structured Datasets; (b) Semantic Mappings and Curated Lexicons; (c) incorporate abstracted, aggregated, or statistically transformed forms of AI-Assisted Outputs into Structured Datasets, Semantic Mappings, and other Platform systems, provided that such use does not reproduce the User's original expressive content in a manner that is substantially similar or identifiable; and (d) AI-generated editorial curation, promotional content, and platform communications.
