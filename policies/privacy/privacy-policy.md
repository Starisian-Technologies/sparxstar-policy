---
version: 0.1.0
date: 2026-04-18
status: drafting
jurisdiction: California
supersedes: none
policyType: module
dependsOnMasterTerms: true
sourceModels:
  - Google Privacy Policy (structural reference — layered disclosure, data use categories)
  - Microsoft Privacy Statement (structural reference — platform/enterprise data model)
  - Stripe Privacy Policy (structural reference — payment data and service-provider framing)
---

# Privacy Policy

> **Layer 1 — Plain Language Summary**
> **Layer 2 — Full Legal Text**
> **Layer 4 — Attestation Record** (added when `status: active`)

*This document is a draft for lawyer review. It has not been reviewed by legal counsel and does not constitute legal advice.*

---


## Role Labels

The following role labels define the legal actors in this document. Where domain-based rendering applies, **SITE OWNER** resolves to the public-facing entity for the active domain. All other labels are fixed.

| Label | Legal Definition | Resolved Identity |
|---|---|---|
| **PLATFORM** | The SPARXSTAR technical platform, infrastructure, APIs, processing systems, AI systems, databases, and hosting environment. | SPARXSTAR (always) |
| **PLATFORM OPERATOR** | The legal entity that owns, operates, and is legally responsible for the PLATFORM. | Starisian Technologies, LLC (always) |
| **SITE OWNER** | The public-facing website owner or site operator responsible for the site on the active domain. | SPARXSTAR on Platform Domains (*.sparxstar.com); the applicable Client Operator on Mapped Custom Domains |
| **END USER** | The visitor, user, customer, attendee, buyer, or other natural person accessing and using the site or service. | Varies by domain and context |
| **SERVICE PROVIDER** | Any third-party contractor, subcontractor, vendor, processor, cloud host, AI provider, payment processor, logistics provider, shipper, printer, moderation service, or support provider engaged by PLATFORM OPERATOR or SITE OWNER to perform backend functions. | Varies |

> **Rendering note:** On Platform Domains (`*.sparxstar.com` and other Company-operated domains), SITE OWNER and PLATFORM OPERATOR are both Starisian Technologies, LLC, and PLATFORM is SPARXSTAR. On Mapped Custom Domains, SITE OWNER resolves to the Client Operator for the end-user-facing layer; PLATFORM and PLATFORM OPERATOR remain unchanged.
>
> In clauses that assign legal responsibility to a specific named entity, the full legal name (Starisian Technologies, LLC) is used in lieu of the PLATFORM OPERATOR label.

### Defined Term Cross-Reference

The following cross-reference maps terms used in the Full Legal Text to their Role Label equivalents:

| Term in Full Legal Text | Role Label Equivalent |
|---|---|
| "the Company" / "Company" | PLATFORM OPERATOR (Starisian Technologies, LLC) |
| "the Platform" / "Platform" | PLATFORM (SPARXSTAR infrastructure) |
| "Client Operator" / "Operator" | SITE OWNER (on Mapped Custom Domains) |
| "User" / "End User" | END USER |
| "Subcontractor" | SERVICE PROVIDER |

---
> **Privacy note:** PLATFORM OPERATOR collects platform-level data on all domains (including Mapped Custom Domains) for security, fraud prevention, and platform operations. SITE OWNER is a separate data controller for the end-user relationship on their domain.

---

## Plain Language Summary

*Layer 1 — Human Readable. Written at Grade 8 reading level. Uses "you" and "we". Short sentences. No passive voice where avoidable.*

> What this policy means for you.

> **Who is "we" in this document?**
> **"We"** and **"us"** in this Privacy Policy refer to **PLATFORM OPERATOR** (Starisian Technologies, LLC) collecting and processing data at the PLATFORM layer.
> On Mapped Custom Domains, SITE OWNER (the Client Operator) is a separate data controller for the end-user relationship on their domain. This policy covers what PLATFORM OPERATOR collects as the PLATFORM provider.

**What PLATFORM OPERATOR does:** PLATFORM OPERATOR collects information about how END USERS use the SPARXSTAR PLATFORM so it can operate the PLATFORM, keep it secure, and improve it.

**What you can do:** END USERS can access, correct, delete, or download personal data, and can opt out of marketing communications at any time.

**What is not allowed:** PLATFORM OPERATOR does not sell END USER personal data to third parties for their own marketing purposes.

---

When you use SPARXSTAR — or any website powered by our platform — we collect some information about you. This helps us run the platform, protect it from abuse, and make it better for everyone.

We collect information in three main ways. First, you give us information directly when you sign up, fill out forms, make purchases, or contact support. Second, we collect information automatically when you use the platform, such as your device type, browser, IP address, and which pages you visit. Third, other companies we work with — like payment processors and analytics providers — may share information with us as part of delivering our services.

We use your information to deliver services, process transactions, keep the platform safe, personalize your experience, and send you relevant communications. We may also use your content and data to train and improve our AI systems. If you do not want us to send you marketing messages, you can opt out at any time using the unsubscribe link in any email or by updating your account preferences.

We share your information with companies that help us operate the platform — such as cloud hosting providers, payment processors, shipping companies, and analytics tools. We do not sell your personal data to third parties for their own marketing. Some websites powered by our platform are run by separate businesses called "operators." Those operators may collect their own data about you. This policy covers what we collect as the platform provider.

If you are in California, you have specific rights under California privacy law. If you are in the European Union or United Kingdom, an EU/UK privacy overlay applies. We take children's privacy seriously and do not collect data from children under 13 without a parent's permission.

---

## Full Legal Text

*Layer 2 — Legally Enforceable. In the event of any conflict between this section and the Plain Language Summary above, this section governs.*

---

### 1. Definitions

Capitalized terms used but not defined in this Privacy Policy have the meanings given to them in the Master Terms of Service. The following additional terms apply:

**1.1 "Personal Data"** means any information that identifies, relates to, describes, is reasonably capable of being associated with, or could reasonably be linked, directly or indirectly, to a natural person, as defined or recognized under Applicable Law, including but not limited to the California Consumer Privacy Act ("CCPA") and the General Data Protection Regulation ("GDPR").

**1.2 "Processing"** means any operation performed on Personal Data, whether or not by automated means, including collection, recording, organization, structuring, storage, adaptation, retrieval, consultation, use, disclosure, dissemination, restriction, erasure, or destruction.

**1.3 "Data Controller"** means the entity that determines the purposes and means of Processing Personal Data, as defined under applicable data protection law.

**1.4 "Data Processor"** means an entity that Processes Personal Data on behalf of a Data Controller, as defined under applicable data protection law.

**1.5 "Service Provider"** means a company or individual that Processes Personal Data on behalf of the Company, subject to written data processing obligations consistent with applicable law.

**1.6 "Platform Data"** means data collected by the Company in connection with the operation, security, analytics, and improvement of the Platform, including data collected across all domains powered by Platform infrastructure.

**1.7 "Operator Data"** means data collected by a Client Operator through their own site or application for their own purposes and under their own privacy disclosures.

**1.8 "Sensitive Personal Data"** means categories of Personal Data that receive heightened protection under Applicable Law, including but not limited to government-issued identifiers, financial account data, health or medical data, biometric data, precise geolocation, racial or ethnic origin, religious beliefs, and data about minors.

---

### 2. Scope and Applicability

**2.1 This Policy.** This Privacy Policy describes how the Company collects, uses, shares, retains, and protects Personal Data in connection with the Platform and Services.

**2.2 Module Status.** This Privacy Policy is a module that supplements and is incorporated into the Master Terms of Service. In the event of conflict between this Privacy Policy and the Master Terms of Service, the Master Terms of Service govern except where this Privacy Policy expressly provides otherwise with respect to data protection matters.

**2.3 Platform-Level Scope.** This Privacy Policy applies to:
  (a) Users who access the Platform directly through Platform Domains;
  (b) Users and End Users who access the Platform through Mapped Custom Domains, with respect to Platform Data collected at the infrastructure layer; and
  (c) Client Operators with respect to their Account data and data submitted in connection with their operator relationship with the Company.

**2.4 Client Operator Data.** Client Operators are independent Data Controllers for the Personal Data they collect from their End Users through their sites. Client Operators are responsible for providing their own privacy disclosures to End Users and for complying with applicable data protection laws in connection with their own data collection and use. This Privacy Policy does not govern Operator Data.

**2.5 Domain-Based Data Collection.**

  (a) **Platform Domains:** The Company collects Platform Data in its capacity as platform operator and Data Controller. All data disclosures in this Privacy Policy apply in full.

  (b) **Mapped Custom Domains:** The Company collects Platform Data at the infrastructure layer (including security logs, fraud detection signals, platform analytics, and AI processing data) regardless of which domain the User is accessing. The Company is the Data Controller for Platform Data. The Client Operator is the Data Controller for Operator Data it collects through its own site.

  (c) End Users accessing a site through a Mapped Custom Domain may be subject to the Client Operator's privacy policy for Operator Data in addition to this Privacy Policy for Platform Data.

---

### 3. Data the Company Collects

**3.1 Account and Registration Data.** When You create an Account, the Company collects information such as: name; email address; username; password (stored in hashed form); profile information You choose to provide; and the date and time of account creation. Client Operators may require additional registration fields for their sites.

**3.2 Transaction and Commerce Data.** When You make a purchase or conduct a financial transaction through the Platform or a Client Operator site, the Company collects: order details; product or service descriptions; pricing; billing address; shipping address; transaction identifiers; and payment method type (e.g., card type, last four digits). Full payment card numbers and full financial account credentials are processed by third-party payment processors and are not stored by the Company on its servers.

**3.3 Content Data.** The Company collects User Content You submit, upload, publish, or otherwise provide to or through the Platform, including text, images, audio, video, documents, and associated metadata such as creation date, file type, and tagging information.

**3.4 Communications Data.** The Company collects the content of messages, inquiries, support tickets, form submissions, and other communications You send to the Company or to other Users through Platform communications features.

**3.5 Usage and Activity Data.** The Company automatically collects data about Your interactions with the Platform, including: pages viewed; features accessed; search queries; clicks; scrolls; navigation paths; time spent; content interactions; and feature usage patterns.

**3.6 Device and Technical Data.** The Company automatically collects technical information about the device and software You use to access the Platform, including: IP address; device type and identifiers; operating system and version; browser type and version; screen resolution; language preferences; referring URLs; and network information.

**3.7 Location Data.** The Company collects coarse location information derived from IP address. The Company does not collect precise GPS or device-level geolocation without Your express consent.

**3.8 Cookies and Tracking Data.** The Company uses cookies, pixel tags, web beacons, local storage, and similar technologies to collect data about Your interactions with the Platform. See the Cookie Policy for detailed disclosures.

**3.9 AI Interaction Data.** Where You interact with AI-assisted features on the Platform, the Company collects the inputs You provide and the outputs generated, together with interaction metadata. This data may be used for AI system operation, quality assurance, safety monitoring, and AI model improvement as described in Section 5.7.

**3.10 Third-Party Source Data.** The Company may receive Personal Data about You from third parties, including: social login providers (if You choose to log in via a third-party account); payment processors; fraud detection providers; analytics partners; and Client Operators (with respect to Account data provided on their behalf).

---

### 4. Legal Basis for Processing (GDPR-Applicable Jurisdictions)

*Note: This section applies to Users subject to the GDPR or UK GDPR. A full EU/UK privacy overlay is pending and will be published as a supplemental document. The following provides the Company's anticipated legal bases.*

**4.1 Contract Performance.** Processing necessary to perform or prepare to perform the agreement with You, including providing the Platform and Services, processing transactions, and maintaining Your Account.

**4.2 Legitimate Interests.** Processing for the Company's legitimate interests, including platform security, fraud prevention, abuse detection, AI system improvement, analytics, and business operations, provided such interests are not overridden by Your data protection rights.

**4.3 Legal Obligation.** Processing required to comply with Applicable Law, including law enforcement requests, regulatory requirements, tax obligations, and legal claims.

**4.4 Consent.** Processing for which the Company has obtained Your express consent, including certain marketing communications and certain AI processing activities where consent is required by Applicable Law.

---

### 5. How the Company Uses Personal Data

**5.1 Platform Operations.** The Company uses Personal Data to operate, maintain, and provide the Platform and Services, including to: authenticate Users; process transactions; deliver content and features; manage Accounts; provide customer support; and fulfill orders and service requests.

**5.2 Security and Fraud Prevention.** The Company uses Personal Data to: detect and prevent unauthorized access, fraud, abuse, and security threats; monitor for violations of these Terms; investigate suspected misconduct; and protect the security and integrity of the Platform.

**5.3 Analytics and Improvement.** The Company uses Personal Data to: analyze usage patterns and Platform performance; identify and fix bugs; develop new features; improve existing features; conduct research; and measure the effectiveness of Platform operations.

**5.4 Personalization.** The Company uses Personal Data to: personalize Your experience on the Platform; recommend relevant content, products, or features; and tailor platform interfaces to Your preferences.

**5.5 Communications.** The Company uses Personal Data to: send transactional communications (such as account confirmations, order confirmations, and support responses); send service notifications and policy updates; and send marketing communications where You have not opted out. You may opt out of marketing communications at any time by following the unsubscribe instructions in any marketing message or by updating Your Account preferences.

**5.6 Legal Compliance.** The Company uses Personal Data as required to: comply with Applicable Law; respond to legal process; enforce these Terms and other agreements; and protect the rights, property, and safety of the Company, its Users, and the public.

**5.7 AI Processing.** The Company may use Personal Data, including User Content, to: operate AI-assisted platform features; conduct automated moderation and safety review; train, fine-tune, and improve AI and machine learning models operated by or on behalf of the Company; generate structured datasets, semantic mappings, and AI outputs; and conduct quality assurance for AI systems. Where required by Applicable Law, the Company will provide notice of and, where applicable, obtain consent for specific AI processing activities. Users may have rights to opt out of certain AI processing activities; see Section 9 (User Rights) for details.

**5.8 Platform-Level Analytics Across Domains.** The Company conducts platform-level analytics across all domains using Platform infrastructure, including Mapped Custom Domains, for the purpose of monitoring platform health, security, and performance. This analytics activity is conducted in the Company's capacity as platform operator and Data Controller for Platform Data.

---

### 6. How the Company Shares Personal Data

**6.1 Service Providers.** The Company shares Personal Data with Service Providers who perform functions on the Company's behalf, including but not limited to: cloud hosting and infrastructure providers; payment processors; email and communications service providers; fraud detection and security vendors; analytics providers; AI and machine learning service providers; customer support platforms; logistics, shipping, and fulfillment providers; printing and production vendors; and content delivery networks. Service Providers are contractually required to use Personal Data only for the purpose of providing services to the Company and to maintain appropriate data security.

**6.2 Client Operators.** The Company may share Account and Content data with the applicable Client Operator where a User has registered or submitted content through that Client Operator's site. Client Operators are bound by data protection obligations in their agreements with the Company.

**6.3 Business Transfers.** If the Company is involved in a merger, acquisition, reorganization, sale of assets, financing, or similar transaction, Personal Data may be transferred as part of that transaction, subject to the acquirer agreeing to privacy commitments consistent with this Privacy Policy.

**6.4 Legal Disclosures.** The Company may disclose Personal Data when: (a) required to do so by Applicable Law or legal process; (b) requested by law enforcement or government authorities; (c) necessary to protect the rights, property, or safety of the Company, its Users, or the public; or (d) necessary to enforce these Terms or other agreements.

**6.5 Aggregated and De-Identified Data.** The Company may share aggregated, anonymized, or de-identified data that does not reasonably identify any individual with partners, researchers, and the public for research, business, and analytics purposes.

**6.6 No Sale for Third-Party Marketing.** The Company does not sell Personal Data to third parties for their own independent marketing or advertising purposes. To the extent any data sharing activity constitutes a "sale" or "sharing" of Personal Data under the CCPA or similar law, the Company provides the opt-out rights described in Section 9.

---

### 7. Data Retention

**7.1 Retention Principles.** The Company retains Personal Data for as long as necessary to fulfill the purposes for which it was collected, including to satisfy legal, regulatory, tax, accounting, or reporting obligations, and to enforce or defend legal claims.

**7.2 Account Data.** Account data is retained for the duration of the active Account and for a reasonable period thereafter to allow for account reactivation requests, to comply with legal obligations, and to defend against claims. Where an Account deletion request is submitted, the Company will delete or de-identify Account data within the timeframe required by Applicable Law.

**7.3 Transaction Records.** Records of financial transactions are retained for the period required by applicable tax and financial recordkeeping laws, typically not less than seven (7) years.

**7.4 AI Training Data.** Where User Content has been incorporated into AI training datasets or contributed to derived AI outputs, those derived works and outputs may be retained beyond the deletion of the original User Content, subject to applicable law and the license granted in the Master Terms of Service.

**7.5 Log Data.** Security logs, access logs, and audit logs are retained for the period required for security monitoring, fraud investigation, and legal compliance purposes, typically between ninety (90) days and two (2) years depending on log type.

**7.6 Communications.** Support communications and contact form submissions are retained for a reasonable period to allow follow-up and to document the history of the support relationship.

---

### 8. Data Security

**8.1 Security Measures.** The Company implements appropriate technical and organizational security measures designed to protect Personal Data against unauthorized access, disclosure, alteration, loss, or destruction. These measures include, without limitation: encryption of data in transit and at rest; access controls and authentication requirements; regular security assessments; and incident response protocols.

**8.2 No Absolute Guarantee.** No security system is impenetrable. The Company cannot guarantee the absolute security of Personal Data. You are responsible for maintaining the security of Your Account credentials and for notifying the Company promptly of any suspected security breach.

**8.3 Breach Notification.** In the event of a Personal Data breach that requires notification under Applicable Law, the Company will provide notification in accordance with its legal obligations.

---

### 9. User Rights

**9.1 Rights Summary.** Subject to Applicable Law and the limitations set forth below, You may have the following rights with respect to Personal Data the Company holds about You:

  (a) **Access:** The right to request confirmation of whether the Company Processes Your Personal Data and to receive a copy of that data.

  (b) **Correction:** The right to request correction of inaccurate or incomplete Personal Data.

  (c) **Deletion:** The right to request deletion of Your Personal Data, subject to legal retention obligations and the survival of derived works described in Section 7.4.

  (d) **Portability:** The right to receive Your Personal Data in a structured, commonly used, machine-readable format, where Processing is based on consent or contract.

  (e) **Objection:** The right to object to Processing of Your Personal Data based on legitimate interests or for direct marketing purposes.

  (f) **Restriction:** The right to request restriction of Processing in certain circumstances.

  (g) **Withdrawal of Consent:** Where Processing is based on consent, the right to withdraw consent at any time without affecting the lawfulness of prior Processing.

**9.2 How to Exercise Rights.** To exercise these rights, submit a request through the Company's designated data rights request channel, identified on the Company's website. The Company will respond within the timeframe required by Applicable Law.

**9.3 Verification.** The Company may require verification of Your identity before fulfilling a rights request, to protect against unauthorized disclosure or deletion.

**9.4 Limitations.** The Company may deny or limit rights requests where Applicable Law permits, including where Processing is necessary for legal compliance, legal claims, platform security, or fraud prevention.

---

### 10. California Privacy Rights (CCPA)

**10.1 Applicability.** This Section applies to California residents as defined under the California Consumer Privacy Act of 2018, as amended by the California Privacy Rights Act of 2020 (collectively, "CCPA").

**10.2 Categories of Personal Information Collected.** The Company collects the following categories of personal information as defined under the CCPA: identifiers; personal records; commercial information; internet or other electronic network activity information; geolocation data (coarse); professional or employment-related information (where provided); education information (where provided); and inferences drawn from the above.

**10.3 Business or Commercial Purposes.** Personal information is used for the business and commercial purposes described in Section 5 of this Privacy Policy.

**10.4 Categories of Third Parties.** Personal information may be shared with Service Providers, Client Operators, and other parties as described in Section 6.

**10.5 Right to Know.** California residents have the right to request disclosure of the categories and specific pieces of personal information the Company has collected, the categories of sources from which it was collected, the business or commercial purposes for collecting or selling it, and the categories of third parties with whom it is shared.

**10.6 Right to Delete.** California residents have the right to request deletion of personal information the Company has collected, subject to applicable exceptions.

**10.7 Right to Correct.** California residents have the right to request correction of inaccurate personal information.

**10.8 Right to Opt Out of Sale or Sharing.** The Company does not sell personal information for monetary consideration. To the extent any data sharing activity constitutes a "sale" or "sharing" for cross-context behavioral advertising under the CCPA, California residents may opt out by submitting an opt-out request through the Company's designated channel.

**10.9 Right to Limit Use of Sensitive Personal Information.** California residents have the right to limit the Company's use of sensitive personal information to the uses permitted under the CCPA.

**10.10 Non-Discrimination.** The Company will not discriminate against You for exercising any of Your CCPA rights.

**10.11 Authorized Agent.** California residents may designate an authorized agent to submit CCPA requests on their behalf. The Company may require verification of the agent's authority before processing the request.

**10.12 Shine the Light.** California Civil Code Section 1798.83 permits California residents to request information about the personal information the Company has shared with third parties for their direct marketing purposes in the preceding year. The Company does not share personal information with third parties for their own direct marketing purposes.

---

### 11. GDPR and EU/UK Privacy Rights — Placeholder

**11.1 Pending Overlay.** A full EU/UK GDPR and UK GDPR privacy overlay is in preparation and will be published as a separate supplemental document. Until that overlay is published, the Company will handle EU/UK User rights requests in accordance with applicable law and to the best of its ability using the procedures described in Section 9.

**11.2 International Data Transfers.** The Company may transfer Personal Data to countries outside the European Economic Area or the United Kingdom. Where such transfers occur, the Company will implement appropriate safeguards as required by Applicable Law, such as standard contractual clauses or equivalent mechanisms.

---

### 12. International Data Transfers

**12.1 Cross-Border Transfers.** The Company operates in the United States and may store or process Personal Data in the United States or in other countries where the Company or its Service Providers maintain facilities. By using the Platform, You understand that Personal Data may be transferred to and processed in countries other than Your country of residence, including countries that may not provide the same level of data protection as Your home country.

**12.2 Transfer Safeguards.** Where Personal Data is transferred from the European Economic Area, the United Kingdom, or Switzerland to countries not recognized as providing an adequate level of data protection, the Company will implement appropriate safeguards as required by Applicable Law. Safeguards may include: standard contractual clauses approved by the European Commission or UK Information Commissioner's Office; binding corporate rules; adequacy decisions; or other mechanisms recognized under applicable data protection law.

**12.3 African Jurisdictions.** Personal Data processed in connection with AI West Africa (AIWA) and other African operational activities may be transferred to or from Gambian or other African jurisdictions. The Company will apply appropriate safeguards consistent with applicable data protection laws in those jurisdictions.

**12.4 U.S. Data Practices.** Personal Data stored or processed in the United States is subject to United States law, including potential disclosure to law enforcement or government agencies under applicable U.S. legal frameworks.

---

### 13. Data Minimization and Privacy by Design

**13.1 Data Minimization Principle.** The Company seeks to limit collection of Personal Data to what is adequate, relevant, and reasonably necessary for the purposes described in this Privacy Policy. The Company does not collect Personal Data speculatively or for undefined future use.

**13.2 Privacy by Design.** The Company integrates privacy considerations into the design and development of new features and services, including: default settings that protect User privacy; access controls that limit internal access to Personal Data on a need-to-know basis; data retention schedules reviewed against business need; and regular privacy impact assessments for high-risk processing activities.

**13.3 De-Identification.** Where the Company processes Personal Data for analytics, research, or AI training purposes, it uses de-identification, pseudonymization, or aggregation techniques to reduce identifiability where practicable, consistent with the purposes for which the data is processed.

**13.4 Sensitive Data Caution.** The Company applies additional safeguards to Sensitive Personal Data, including additional access controls, enhanced encryption, and heightened data minimization practices. Users are advised not to submit Sensitive Personal Data to the Platform unless expressly required for the purpose of a specific Service.

**13.5 Purpose Limitation.** The Company uses Personal Data only for the purposes for which it was collected or for compatible purposes. Where the Company wishes to use Personal Data for a materially different purpose, it will provide additional notice and, where required by Applicable Law, obtain additional consent.

---

### 14. Gambia and African Operations — AIWA

**14.1 Gambia and African Operations.** AI West Africa (AIWA), a Gambian-based initiative within the Starisian Technologies ecosystem, may operate services within The Gambia and other African jurisdictions. A jurisdiction-specific privacy overlay addressing applicable data protection laws in relevant African jurisdictions, including the Gambia Data Protection Act, is in preparation and will be published as a supplemental document.

**14.2 Data Localization.** To the extent required by applicable African data protection law, including data localization requirements in relevant jurisdictions, the Company will implement appropriate data storage and processing arrangements.

**14.3 Cross-Border Transfers from Africa.** Cross-border transfers of Personal Data from African jurisdictions will be conducted in accordance with the applicable data protection law of the originating jurisdiction. Where the applicable law requires government approval, contractual safeguards, or other mechanisms for cross-border transfers, the Company will comply with those requirements.

---

### 15. Children's Privacy

**15.1 Age Restriction.** The Platform and Services are not directed to children under the age of thirteen (13). The Company does not knowingly collect Personal Data from children under thirteen (13) without verifiable parental consent in compliance with the Children's Online Privacy Protection Act ("COPPA") and applicable law.

**15.2 Discovery and Deletion.** If the Company becomes aware that it has collected Personal Data from a child under thirteen (13) without verifiable parental consent, it will take steps to delete that data promptly. Parents or guardians who believe their child has provided Personal Data to the Company without consent should contact the Company using the contact information in Section 16.

**15.3 Users Aged 13–17.** Users between the ages of thirteen (13) and seventeen (17) may only use the Platform with the consent and supervision of a parent or legal guardian as described in the Master Terms of Service.

---

### 16. Automated Decision-Making and Profiling

**16.1 Automated Processing.** The Company may use automated processing, including machine learning and rule-based systems, in connection with platform operations. Automated processing activities include but are not limited to: spam and abuse detection; fraud and unauthorized-access detection; content moderation pre-screening; personalization of Platform features; and AI-assisted quality review of User Content.

**16.2 No Solely Automated Legal Decisions.** The Company does not make decisions that produce legal effects on Users or that significantly affect Users based solely on automated processing, except where: (a) such processing is authorized by Applicable Law; (b) is necessary for entering into or performing a contract; or (c) is based on explicit consent. Where automated decisions are used for Account enforcement actions, the Company provides a human review pathway, described in Section 14.3 of the Master Terms of Service.

**16.3 GDPR Automated Decision Rights.** For Users in the EU/UK where the GDPR or UK GDPR applies, You have the right to: request human review of any automated decision that produces significant effects; express Your point of view regarding the automated decision; and contest the automated decision. These rights are subject to exceptions under applicable law. To exercise these rights, submit a request through the data rights channel described in Section 9.2.

**16.4 AI Profiling.** The Company may build anonymized or pseudonymized behavioral profiles of Platform usage for the purposes of improving the Platform and personalizing User experience. Such profiles do not constitute decisions about individuals in a legal or regulatory sense but are used to optimize platform recommendations and features. Users may object to this type of profiling under rights described in Section 9.

---

### 17. Marketing and Communications

**17.1 Types of Communications.** The Company may send the following categories of communications to Users:

  (a) *Transactional communications:* Account confirmations, password reset messages, order confirmations, transaction receipts, support responses, and service notifications. These are sent as part of the provision of Services and do not require separate consent.

  (b) *Service-update communications:* Policy updates, Terms changes, security alerts, and platform maintenance notices. These are sent as necessary to inform Users of material changes to the Services.

  (c) *Marketing communications:* Promotional messages, product announcements, feature highlights, event notices, and similar commercial communications. Marketing communications are sent on the basis of User consent or, where permitted by Applicable Law, on the basis of a soft opt-in from existing customers.

**17.2 Opt-Out of Marketing.** You may opt out of marketing communications at any time by: (a) clicking the unsubscribe link included in any marketing email; (b) updating Your communication preferences in Your Account settings; or (c) submitting an opt-out request through the Company's designated contact channel. The Company will process opt-out requests within the timeframe required by Applicable Law. Opting out of marketing does not affect transactional or service-update communications.

**17.3 Third-Party Marketing.** The Company does not sell Your Personal Data to third parties for their independent marketing use. The Company may use third-party email delivery services and marketing tools as Service Providers to send Company marketing communications on the Company's behalf.

**17.4 SMS and Push Notifications.** Where the Platform supports SMS or push notification communications, the Company will obtain Your explicit consent before sending marketing messages through those channels. You may withdraw consent at any time by unsubscribing from those channels through Your Account settings or by following the opt-out instructions provided in the relevant messages.

---

### 18. Third-Party Analytics and Advertising Partners

**18.1 Analytics Partners.** The Company uses third-party analytics services to collect and analyze data about Platform usage. Analytics partners receive aggregated or pseudonymized data about User interactions with the Platform. The Company takes reasonable steps to ensure analytics partners do not use this data for their own independent purposes beyond providing analytics services to the Company.

**18.2 Advertising Technology.** Where the Platform integrates advertising technology, advertising partners may collect data through Cookies and Similar Technologies as described in the Cookie Policy. Advertising partners may use this data to display targeted advertisements on the Platform and on third-party sites.

**18.3 Social Media Integrations.** Where the Platform includes social media sharing buttons, embedded content, or social login features, the applicable social media platform may collect data about Your interaction with those features. The collection and use of data by social media platforms is governed by their own privacy policies, which the Company does not control.

**18.4 Payment Processors.** Payment transactions are processed by third-party payment processors. Payment processors receive transaction data necessary to complete and verify payments, manage fraud, and comply with financial regulations. Payment processor data use is governed by their own privacy policies. The Company does not store full payment card numbers.

**18.5 AI and Cloud Providers.** The Company uses cloud infrastructure providers and AI service providers to host the Platform and to provide AI processing capabilities. These providers may process Personal Data in the course of providing infrastructure and AI services to the Company. They are engaged as Service Providers under data processing agreements that restrict their use of Personal Data to providing services to the Company.

---

### 19. Support and Contact — Forms-First Posture

**19.1 Support Channels.** The Company provides support and handles inquiries primarily through structured forms, ticketing systems, and designated online channels. Direct personal contact information for Company staff is not broadly published in order to maintain security and manage volume effectively.

**19.2 Data Rights Requests.** Requests to exercise data rights described in Sections 9 through 12 should be submitted through the Company's designated data rights request channel, identified on the Company's website.

**19.3 Privacy Inquiries.** General privacy inquiries, questions about this Privacy Policy, or concerns about Company data practices should be submitted through the privacy inquiry channel identified on the Company's website.

---

### 20. Changes to This Privacy Policy

**20.1 Amendment Right.** The Company reserves the right to amend, update, or replace this Privacy Policy at any time. The Company will provide notice of material changes by: (a) updating the version number and date in the document header; (b) posting notice on the Platform; or (c) sending notice to the email address associated with Your Account, at the Company's discretion.

**20.2 Continued Use.** Your continued use of the Platform or Services after the effective date of any amendment to this Privacy Policy constitutes Your acknowledgment of the amended Privacy Policy. If You do not agree to the amended Privacy Policy, You must stop using the Platform and Services.

**20.3 Version History.** Prior versions of this Privacy Policy are maintained in the Company's policy version repository for reference.

**20.4 Retroactive Application.** Material amendments to this Privacy Policy do not apply retroactively to data collected under prior versions of the Policy, except where required by Applicable Law or where the new version provides greater protection for User privacy.

**20.5 Material Change Definition.** For purposes of this Section, a "material change" includes any change that: (a) expands the categories of Personal Data collected; (b) changes the purposes for which Personal Data is used in a manner adverse to Users; (c) changes the categories of third parties with whom Personal Data is shared; or (d) reduces User rights under this Privacy Policy.

---

### 21. Contact

**Starisian Technologies, LLC**
Legal Department / Privacy Team
[Contact information — to be completed before publication]

For privacy inquiries, please use the official privacy contact form or designated privacy inquiry channel published on the Company's website.

---

## Attestation Record

*Layer 4 — Added when `status` is promoted to `active`. Do not populate during drafting or review.*

| Field | Value |
|---|---|
| Version | — |
| SHA-256 Hash | — |
| Attested Date | — |
| Attestation Method | — |
| Attested By | — |

---

*This document is a draft for lawyer review. It has not been reviewed by legal counsel and does not constitute legal advice. All policy documents must be reviewed and approved by qualified legal counsel before publication.*
